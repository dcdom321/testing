...
def _fetch_actual_vc(loc, date_str):
    station = loc["station"]
    unit = loc["unit"]
...
def _fetch_actual_wapi(loc, date_str):
    unit = loc["unit"]
    q = f"{loc['lat']},{loc['lon']}"
    url = (
        f"https://api.weatherapi.com/v1/history.json"
        f"?key={WAPI_KEY}&q={q}&dt={date_str}"
    )
    data = requests.get(url, timeout=(5, 8)).json()
    days = data.get("forecast", {}).get("forecastday", [])
    if days:
        day = days[0].get("day", {})
        key = "maxtemp_f" if unit == "F" else "maxtemp_c"
        v = day.get(key)
        if v is not None:
...
def get_actual_temp(city_slug, date_str):
    """Actual temperature for closed markets. Tries the configured provider first,
    then falls back to the other if a key is available."""
    loc = LOCATIONS[city_slug]
    order = (
        [("WAPI", WAPI_KEY, _fetch_actual_wapi), ("VC", VC_KEY, _fetch_actual_vc)]
        if WEATHER_PROVIDER == "weatherapi"
        else [("VC", VC_KEY, _fetch_actual_vc), ("WAPI", WAPI_KEY, _fetch_actual_wapi)]
    )
    for name, key, fn in order:
        ...
        try:
            ...
            print(f"  [{name}] {city_slug} {date_str}: {e}")
    return None
